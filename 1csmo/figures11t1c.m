a1=load('C:\Users\salma\MATLAB Drive\T11 MHE\1csmo\dsfhm11t1csscec.mat');
a2=load('C:\Users\salma\MATLAB Drive\T11 MHE\1csmo\dsfhm11t1cd2c.mat');
dsfhm11t1cssce=a1.dsfhm11t1csscec;
 dsfhm11t1cd2=a2.dsfhm11t1cd2c;
% initial11t1c;
...
%t=1:length(dsfhm11t1cd2.v);
hold on
plot(dsfhm11t1cd2.v(1:length(dsfhm11t1cd2.v)-1))
ylabel('Voltage[V]')
yyaxis right
error=(dsfhm11t1cd2.v(1:length(dsfhm11t1cd2.v)-1)-...
    dsfhm11t1cssce.y(1:length(dsfhm11t1cd2.v)-1));
rmsv=(sqrt(sum(error.^2)/length(error)))*(100/mean(dsfhm11t1cssce.y))
variance=var(error)
plot(error(1,4:end),'--')
...
figure
yyaxis left
plot(dsfhm11t1cssce.socp,'--')
t=1:length(dsfhm11t1cd2.v);
hold on
plot(dsfhm11t1cd2.socp(:,p.N:end),'--')
hold on
plot(p.socr,'-')
...
error1=(dsfhm11t1cd2.socp-...
    dsfhm11t1cssce.socp);
rmsc=(sqrt(sum(error1.^2)/length(error1)))*(100/mean(dsfhm11t1cd2.socp(:,3:end)))
...
%%
 figure
% plot(abs(dsfhm11t1cssce.u(1,3:end)*p.a),'-.')
