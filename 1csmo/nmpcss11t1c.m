  ...
  %  [ds,w] = plant(ds,w);
   w.tc=w.t1;
    %% electrolyte model for estimator
     da = fhmss11t1csslyte(d.qf(:,end),w);
...
p.ctr(p.ij)=p.crt;
d1= fhmss11t1cssb1(d,p);
d.x1=d1.x;d.x0=d.x1;
 end
function cost = costfe(w,da,x)
        y = fhmss11t1cssb(x,da,w);
        v=y.y;
...
d.x=[d.x;fm(d1.x)];d.est=[d.est,w.est];
