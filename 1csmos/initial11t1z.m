addpath(genpath('C:\Users\salma\MATLAB Drive\T11 MHE\1csmos') );
...
p.ecl=[1e-6, 5*p.ce];
 [phisn1,phisp1]=ocp11t1z(xnn,xpp);
...
p.y0.opn=0;p.y0.opns1=phisn1;p.y0.jsn1=0;p.y0.up=phisp1;
p.y0.x=[cpp;bc;ab;p.y0.jsn1]';p.y0.y=phisp1-phisn1;
p.y0.x0=cpp;
%p.y0.x=[cpp;bc;ab;]';
p.nx=length(p.y0.x);p.y0.y=phisp1-phisn1;
p.y0.socn=p.soci;p.y0.soh=ab/p.c;p.y0.rfilm=p.rsei;
p.y0.u0= p.ua*ones(1,p.M);
